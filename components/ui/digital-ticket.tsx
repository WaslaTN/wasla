...
import ETDDisplay from './etd-display'
    ...
    return (
      <div className="relative w-full h-[400px]" style={{ perspective: '1000px' }}>
        ...
        >
          ...
          >
            <Card className="w-full h-full bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-2 border-blue-500/30 transition-all duration-300 overflow-hidden hover:border-blue-400/50">
              <CardContent className="p-4 h-full flex flex-col">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className={`px-2 py-0.5 text-xs ${getStatusColor(booking.status)}`}>
                          {getStatusIcon(booking.status)}
                          <span className="ml-1">{booking.status}</span>
                        </Badge>
                      </div>
                      <div className="text-white/60 text-xs flex items-center gap-4">
                        <span>{booking.verificationCode}</span>
                        <span>{booking.seatsBooked} {booking.seatsBooked > 1 ? 'seats' : 'seat'}</span>
                        {showETD && timeLeft && (
                  ...
                  <div className="text-right">
                    <div className="text-lg font-bold text-white">{booking.totalAmount} DT</div>
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={(e) => {
                ...
                <div className="flex-1 flex items-end">
                  <div className="text-xs text-slate-400 w-full text-center">Tap to flip</div>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          >
            <Card className="w-full h-full bg-gradient-to-r from-purple-600/20 to-pink-600/20 border-2 border-purple-500/30 transition-all duration-300 overflow-hidden hover:border-purple-400/50">
              <CardContent className="p-4 h-full flex flex-col">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">
                    ...
                    <div>
                      <div className="text-white font-semibold text-sm">Ticket Details</div>
                      <div className="text-white/60 text-xs">Verification Code</div>
                    </div>
                  </div>
                  <Badge className="px-2 py-0.5 text-xs bg-purple-500/20 text-purple-400 border-purple-500/30">
                    BACK
                  </Badge>
                </div>
                ...
                <div className="flex-1 space-y-3">
                  <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-700/50">
                    <div className="text-center">
                      ...
                      <p className="text-white font-mono text-sm">{booking.verificationCode}</p>
                      <p className="text-gray-400 text-xs mt-1">Show this at station</p>
                    </div>
                  </div>

                  <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-700/50">
                    <div className="text-xs text-gray-400 space-y-1">
                      <p>• Valid for {new Date(booking.journeyDate).toLocaleDateString()}</p>
                      <p>• {booking.seatsBooked} seat{booking.seatsBooked > 1 ? 's' : ''} booked</p>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-700/50">
                  ...
                  <div className="text-xs text-slate-400">Tap to flip</div>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="w-full h-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-2 border-blue-500/30 shadow-2xl overflow-hidden relative group hover:border-blue-400/50 transition-all duration-300">
            ...
            <CardContent className="p-6 h-full flex flex-col relative z-10">
              ...
              <div className="flex-1 space-y-6">
                ...
                {/* ETD Info */}
                {showETD && booking.etaPrediction && timeLeft && (
